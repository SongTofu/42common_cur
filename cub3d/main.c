...
/*   Updated: 2021/05/25 15:49:07 by yejsong          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_image(t_game *g, char *path, t_img *img, int i)
{
	...
	g->texture[i] = (int *)ft_calloc(img->img_width * img->img_height, \
	sizeof(int), 0);
...
void	load_texture(t_info *info, t_game *g)
{
	...
	while (++i < 5)
		load_image(g, info->cub_info[i], &img, i);
}
