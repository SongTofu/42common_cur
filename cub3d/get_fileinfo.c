...
/*   Updated: 2021/05/25 15:58:40 by yejsong          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		info_parsing(char *line, t_info *info, int i)
{
	...
	while (++i < 8)
	{
		...
		if (!ft_strncmp((char *)inf[i], line, inf_len) && \
		ft_isspace(*(line + inf_len)) && !info->cub_info[i])
		{
			if (i < 4)
			{
