...
Conversion::Conversion(const Conversion &c) : _charConvOk(c._charConvOk), \
				_intConvOk(c._intConvOk), _floatConvOk(c._floatConvOk), \
				_doubleConvOk(c._doubleConvOk), _charValue(c._charValue), \
				_intValue(c._intValue), _floatValue(c._floatValue), \
				_isLimitBool(c._isLimitBool), _limit(c._limit), _stringError(c._stringError)
{
}
...
Conversion	&Conversion::operator=(const Conversion &c)
{
	if (this != &c)
	{
		_charConvOk = c._charConvOk;
		_intConvOk = c._intConvOk;
		_floatConvOk = c._floatConvOk;
		_doubleConvOk = c._doubleConvOk;
		_charValue = c._charValue;
		_intValue = c._intValue;
		_floatValue = c._floatValue;
		_isLimitBool = c._isLimitBool;
		_limit = c._limit;
		_stringError = c._stringError;
	}
	return (*this);
}
